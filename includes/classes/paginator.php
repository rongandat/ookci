 ...
 function getPageLinks() {
 	 ...
 	 $strPageLinks		=	"";
  	 ...
  	 if($this->getCurrent()==1)
		         {
		       		  $first = "";
		         } else { $first=' <a href="' .  $this->getPageName() . "&".$this->page_param_name."=" . $this->getFirst() . '" ><img src="'.getHtmlImageSource('icon_page_first.gif').'" title="Navigate to the first page" border="0" /></a>'; }  
		       ...
		       if($this->getPrevious())
		         ...
		         } else { $prev='<img src="'.getHtmlImageSource('icon_page_prev.gif').'" title="Navigate to the next page" border="0" />'; }
	
	         if($this->getNext())
		         {
		         $next = ' <a href="'. $this->getPageName() . "&".$this->page_param_name."=" . $this->getNext() . '" ><img src="'.getHtmlImageSource('icon_page_next.gif').'" border="0" /></a> | ';
		         } else { $next='<img src="'.getHtmlImageSource('icon_page_next.gif').'" title="Navigate to the previous page" />'; } 
				 
  			 if($this->getLast())
		         {
		         $last = ' <a href="' . $this->getPageName() . "&".$this->page_param_name."=" . $this->getLast() . '"> <img src="'.getHtmlImageSource('icon_page_last.gif').'"  border="0" /></a>';
		         } else { $last='<img src="'.getHtmlImageSource('icon_page_last.gif').'" title="Navigate to the last page" />'; }
 ...
 function getJavascriptPage($jsparams='') {
 	 ...
 	 $strPageLinks		=	"";
  	 ...
  	 if($this->getCurrent()==1)
		         {
		       		  $first = "";
		         } else { $first=' <a href="javascript:' .  $this->getJavascriptPageName() . "(" . $this->getFirst() . $jsparams.')" ><img src="'.getHtmlImageSource('icon_page_first.gif').'" title="Navigate to the first page" border="0" /></a>'; }  
