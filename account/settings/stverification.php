...
$smarty->assign('view_verification_status', 'N/A');
        $smarty->assign('view_verification_ip', '*****');
if ($action == 'update_account') {
    ...
    if (count($validator->errors) == 0) { // update user personal info	
        ...
        if(!empty($verification_status)){
            $signup_data_array['verification_ip'] = get_client_ip();
        }
...
if ($action == 'view_info') {
    ...
    if (count($validator->errors) == 0) {
        $smarty->assign('view_verification_status', ($account_info['verification_status'] == 1)?'Enable':'Disable');
        $smarty->assign('view_verification_ip', $account_info['verification_ip']);

        $smarty->assign('verification_status', $account_info['verification_status']);
        $smarty->assign('verification_ip', $account_info['verification_ip']);
    }
}
...
//BOF: get countries list
    ...
    //EOF: get zones list
    $smarty->assign('html_content', $smarty->fetch('account/settings/stverification.html'));
} else {
