...
//Nazwan: new added function to show custom login button
...
function onLoginHandler(eventObj) {
	try{
	    var checkCandidateSession = false;
		var filename = window.location.pathname.match(/.*\/([^/]+)\.([^?]+)/i)[1];
		if(filename == "update-account")
			checkCandidateSession = true;
		$.post("../social-login/login.php", { "userObject": JSON.stringify(eventObj), "action" : "socialLogin", "checkCandidateSession" : checkCandidateSession},
			function(data){
				...
				switch(data.success)
				{
					...
					case "SUCCESS": //successfull social login
						if(data.newUser == true){
							if(filename == "update-account") {
								...
								var url2 = location.href.split("/registration/update-account.php");
								var completeUrl = url2[0]+'/registration/update-account.php?la=1';

								if(param != '')
									completeUrl = completeUrl+'&'+param;
								window.location = completeUrl;
							}else{
								// new social login user pop out lightbox
								$.ajax({
									url: '../registration/login-exist.php?li=' + pUserObject.user.email,
									type: 'GET',
									success: function(o){
										var result = o.split("|");
										$('#social_name', '#pnLink').html(pUserObject.user.nickname);
										if(result[1] == "unavailable"){
											// email exist as login id, force link
											$(this).displayLightBox();
											$('input[type="text"], input[type="password"]','#pnLink').val('');
											$('div.errorReg , [type="l"]','#pnLink').hide();
											$('[type="fl"]','#pnLink').show();
										}else{
											var url = window.location.href;
											// registration
											if(filename == "quick-register" || filename == "create-account" || filename == "register-apply" || (filename == "register-login" && url.indexOf("signup=1") != '-1')) {
												url = url.substring(0, -1); // remove # which cause the reload problem
												url += (url.split('?')[1] ? '&':'?') + 'sl=1&site='+$('input[name="site"]').val()+'&language_code='+$('input[name="language_code"]').val();
												if(filename == "register-login")
												{
													url +='&signup='+$('input[name="signup"]').val();
												}
												if(filename == "create-account" || filename == "register-apply"){
													url += '&job_id='+$('input[name="j"]').val() + '&j='+$('input[name="j"]').val()+'&s='+$('input[name="s"]').val()
														+'&a='+$('input[name="a"]').val()+'&source='+$('input[name="source"]').val()
														+'&r='+encodeURIComponent($('input[name="r"]').val())+'&fr='+$('input[name="fr"]').val();

													if($('input[name="oa"]').val()==1)
														url += '&oa='+$('input[name="oa"]').val();
												...
												$(this).displayLightBox();
												$('input[type="text"], input[type="password"]','#pnLink').val('');
												$('div.errorReg , [type="fl"]','#pnLink').hide();
												$('[type="l"]','#pnLink').show();
											}
										}

										$(window).resize( function(){
											if($('#pnLink').is(':visible'))
												$(this).displayLightBox();
										});
								},
								error: function(){}
							...
							// returning social login user, trigger login
							$('input[name="login_id"], input[name="password"]').val('');
							$('#login, form[name="frmLogin"], form[name="frmSubmitLogin"]').append($("<input>").attr("type", "hidden").attr("name", "sl").val("1"))
																						   .append($("<input>").attr("type", "hidden").attr("name", "social_token").val(data.social_token))
																						   .submit();
						}
						break;
					case "INVALID_LINK_USER":
						if(filename == "update-account") {
							...
							var url2 = location.href.split("/registration/update-account.php");
							var completeUrl = url2[0]+'/registration/update-account.php';

							if(data.newUser == false){
								if(window.location.href.indexOf("inv=1") == -1) {
									completeUrl = completeUrl + '?inv=1';
								}
							}
							if(param != '') {
						...
						break;
					case "REDIRECT":
						var dir = $('#login, form[name="frmLogin"], form[name="frmSubmitLogin"]').attr('action').split('?')[0];
						dir = (dir != '') ? dir.split('/') : window.location.pathname.split('/');
						action = data.url + '/' + dir[dir.length - 2] + '/' + dir[dir.length - 1] + '?nrfr=1'; // redirect to remote server for login
						
						$('input[name="login_id"], input[name="password"]').val('');
						if(data.back !== undefined){
							$('#login, form[name="frmLogin"], form[name="frmSubmitLogin"]').append($("<input>").attr("type", "hidden").attr("name", "back").val(data.back));
						}
						
						$('#login, form[name="frmLogin"], form[name="frmSubmitLogin"]').attr('action', action)
																					   .append($("<input>").attr("type", "hidden").attr("name", "sl").val("1"))
																					   .append($("<input>").attr("type", "hidden").attr("name", "social_token").val(data.social_token))
																					   .append($("<input>").attr("type", "hidden").attr("name", "uid").val(data.UID))
																					   .submit();
						break;
					case "MISSING_EMAIL": // The registration is pending.
					default:
						//alert("default, onLoginHandler: " +  data.success);
				}
			}
			, "json");
	}catch(e){}
...
$.fn.displayLightBox = function() {
	$('#backgroundFilter').show();
	$('#pnLink').show( 0, function(){
		$(this).css({
			position :"absolute",
...
var pUserObject = null;

function unlinkSocialLogin(x, msg)
{
	if(confirm(msg))
	{
		document.location.href= "?ul=1" + "&x=" + x;
	}
	return false;
}
function detectBrowser(){
    var appName= navigator.appName, userAgent= navigator.userAgent, tem;
    var browser= userAgent.match(/(opera|chrome|safari|firefox|msie)\/?\s*(\.?\d+)/i);
	var IE8compat= userAgent.match(/Trident\/4\.0/);
	var IE9compat= userAgent.match(/Trident\/5\.0/);
	...
	if(browser[0].toLowerCase() == "firefox" && parseInt(browser[1]) <14 ){
		displayMessage("firefox");
	}else if(browser[0].toLowerCase() == "msie" && parseInt(browser[1]) < 8){
		if(IE8compat || IE9compat){
		}
		else{
		displayMessage("msie");
		}
...
function displayMessage(browserType){
		...
		if(browserType == "firefox"){
			upgradeLink = "http://www.mozilla.org/en-US/products/download.html";
			positionType = "fixed";
		}else if(browserType == "msie"){
			upgradeLink = "http://windows.microsoft.com/en-US/internet-explorer/download-ie";
			document.body.style.height="100%";
			positionType = "absolute";
			var header = document.getElementById("header_navi");
			if(header && header.offsetTop < 38){
		...
		if(aspnetForm){
			aspnetForm.style.marginTop = "40px";
		}
		document.write('<a id="upgradeBrowser" href="'+ upgradeLink+ '" target="_blank" style="position:'+ positionType +';top:0;left:0;z-index:10000;width:100%;background:#fdfabd;line-height:40px;text-align:center;border-bottom:1px solid #ccc;cursor: pointer;"><div style="width:620px; margin:0 auto;"><img src="../../myjs11-static/default/common/image/stop-icon.png" border="0" style="display:block;float:left;"><span style="display:block;float:left;margin-left:10px;">You are using an <strong>older browser</strong>. To ensure compatibility and optimal experience</span><span style="display:block;float:left;margin: 3px 0 0 10px;color:#000; background:#ffff00 url(../../myjs11-static/default/common/image/button-bg.png) repeat-x 0 50%; line-height:14px; padding:8px; border:1px solid #aaa; font-weight:bold; cursor: pointer;-webkit-border-radius: 5px;-moz-border-radius: 5px;border-radius: 5px; ">Upgrade Now</span><div style="clear:both;"></div></div></a>');
}
var button = {
	LOGIN : "Login with Facebook",
	LOGIN_CREATE : "Login & Create with Facebook",
	REGISTRATION : "Connect with Facebook",
	MY_ACCOUNT : "Connect with Facebook",
	SAVE_JOB : "Login & Save Job with Facebook"
};
